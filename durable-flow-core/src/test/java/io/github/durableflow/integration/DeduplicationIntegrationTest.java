...
class DeduplicationIntegrationTest extends BaseIntegrationTest {
    ...
    void differentPayloads_sameSource_notDeduplicated() {
        ...
        ReceiveResult r1 = engine.receive(message("src", "payload-a"), ReceiveOptions.of(wf));
        ReceiveResult r2 = engine.receive(message("src", "payload-b"), ReceiveOptions.of(wf));
    ...
    void samePayload_differentSources_notDeduplicated() {
        ...
        ReceiveResult r1 = engine.receive(message("source-A", "shared-payload"), ReceiveOptions.of(wf));
        ReceiveResult r2 = engine.receive(message("source-B", "shared-payload"), ReceiveOptions.of(wf));
    ...
    void duplicate_returnsOriginalMessageState() throws Exception {
        ...
        ReceiveResult first = engine.receive(message("src", "dup-payload"), ReceiveOptions.of(wf));

        waitFor(3000); // let step execute

        ReceiveResult second = engine.receive(message("src", "dup-payload"), ReceiveOptions.of(wf));
